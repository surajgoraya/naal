...
class Home extends Component<Props, { token: string | undefined, loading: boolean, gitUser: any | undefined, naalContent: any | undefined, editModalOpen: boolean, modalState: Link, currentRepo: string, currentSHA: string }> {
    ...
    async sendModalChange() {
        ...
        let resp = await this.OKit?.request(`PUT ${this.state.currentRepo}`, {
            ...
            committer: { name: `${this.state.gitUser.login} via naal service`, email: 'naal@interfra.me' },
                ...
                <Grid style={{ height: '100%' }} areas={[
                    ...
                    <Box gridArea='content' background='brand'>
                        ...
                        <Main pad="large">
                            ...
                            <Heading style={{ marginTop: 80 }}>Welcome,<Anime opacity={[0, 1]} complete={console.log} loop={false} translateY={'0.1 em'} delay={(e, i) => { return Number(i) * 200 }}>{this.state.gitUser.name}</Anime> </Heading>

                            <Anime opacity={[0, 1]} translateY={'0.5em'} easing="easeOutElastic" delay={(e, i) => { return Number(i) * 700 }}>
                            ...
                            <div>
                                <Anime opacity={[0, 1]} translateY={'1em'} delay={(e, i) => { return Number(i) * 20 }}>
                                    <Heading size="small" color="dark-4">CURRENT</Heading>
                                    {this.state.naalContent == undefined ? <StageSpinner loading={this.state.naalContent == undefined} color='#FD5200' /> : <React.Fragment>
